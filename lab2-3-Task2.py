...
def parse_auth_line(line):
    ...
    #assume ts_str means time stap string, read from first three tokens 0-3, start with 2025 - concatinate the first part of the log to the year 2025
...
'''   # This prints the ip address, and the amount of times that it failed. so len(times) is the amount of times the IP address failed.
    for ip, times in per_ip_timestamps.items():
        print(ip, len(times)) 
        '''
        
''' #This prints the dictionary of times that the specific IP address failed
    for ip, times in per_ip_timestamps.items():
        print(ip, times)
'''
